...
interface PexelsPhoto {
    ...
    avg_color: string | null; // It can be null
    src: {
        original: string;
        large2x: string;
        large: string;
...
export default function EditPostPage() {
    ...
    useEffect(() => {
        const fetchPost = async () => {
            ...
            try {
                ...
                setError('Error charging the post.Verify that you exist and have permits to edit it.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const fetchPexelsImages = async (query: string) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Error in Pexels's response: ${response.status}`);
            }

            const data = await response.json()
            setRecommendedImages(data.photos || [])
        } catch (err) {
            console.error('errorFetchingPexelsImages:', err)
        } finally {
    ...
    useEffect(() => {
        if (title.trim()) {
            //We implement a debounce to do not to make many requests
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Redirect the post page
            router.push(`/posts/${updatedPost.slug}`);
        } catch (err) {
            console.error('errorUpdatingPost:', err);
            setError('Error when updating the post.Try it again.');
        } finally {
            setSaving(false);
        }
    };
        ...
        <div className="container mx-auto px-4 py-16 max-w-3xl">
            <div className="bg-white rounded-lg shadow-md p-6">
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-600">Update your Postly publication</p>
                </div>
                        ...
                        {title.trim() && (
                            <>
                                <div className="mt-4 mb-2">
                                    <h4 className="text-sm font-medium text-gray-700">Suggested images (Pexels)</h4>
                                </div>
